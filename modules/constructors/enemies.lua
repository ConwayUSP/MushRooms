...
function newNuclearCat(spawnPos, room)
	...
	local attack = newNuclearShotAttack(false, 2.5, atksCooldown, 800, straightMovement())
	local attackSlow = newNuclearShotAttack(false, 10.0, atksCooldown, 200, straightMovement())
...
function newSpiderDuck(spawnPos, room)
	...
	local atkCooldown = randCooldown(0.2, 3.0)
	local attack = newPebbleShotAttack(false, 3, atkCooldown, 400, zigZagMovement(1, math.rad(35)))
